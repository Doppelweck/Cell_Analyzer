...
for nol=levels:-1:1
    ...
    for i=1:noi
       ...
       wim = (wim - meanOfWim);%/max(max(wim));% zero-mean image; is useful for brightness change compensation, otherwise you can comment this line
...
%warpedImage = warpedImage;  %%%(uint8(warpedImage);
warp = final_warp;
%%%