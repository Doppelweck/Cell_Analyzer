...
transform = 'translation'; % verschiebung
% transform = 'affine'; % scherung
% transform = 'homography';
% transform = 'euclidean'; %geht nicht?!?!?!?


%YOU MIGHT NEED MORE ITERATIONS FOR HOMOGRAPHIES!

NoI = 50; % number of iterations
NoL = 1;  % number of pyramid-levels %was ist das????

verbose = 1; %plot on off

%ausgangsbild=imread('cameraman.tif'); % ... or try your image
%imwrite(im_demo,'image.pgm')

[A,B,C]=size(ausgangsbild);

if C==3     % RGB zu grayscale bild
    ausgangsbild=rgb2gray(ausgangsbild);
end

ausgangsbild=double(ausgangsbild);
...
switch lower(transform)
    ...
    case 'homography'
        ...
        init=eye(3); % zeros matrix 3x3 mit 1 in der hauptdiagonalen
...
%%%%%%%%%
% im_demo=ausgangsbild
% warp_demo=eingabeparameter
% transform= art der transformation (affin, translation,...)
% Nx,Ny = Breite/Höhe des neuen bildes; abhängig von größe der verschiebung
% linear = gibt interpolationsart vor (siehe interp2)
%%%%%%template_demo = spatial_interp(ausgangsbild, warp_demo, 'linear', transform, Nx, Ny);
...
[results, final_warp, warped_image]=ecc(ausgangsbild, template_demo, NoL, NoI, transform, init);
...
image2 = spatial_interp(double(ausgangsbild), final_warp, 'linear', transform, nx, ny);
...
if verbose
    ...
    image_error=double(image2)-double(template_demo);
